...
public class Environment {
	...
	// look at part 4-2
	public float getAverageDownloadsPerApp(Strategy strategy) {// TODO make sure
																// it isn't the
																// FLEXIBLE
																// strategy
		int totalDownload = 0;
	...
	public float getTop20TotalDownloads(Strategy strategy) {
		...
		for (int i = 0; i < best.length; i++) {
			if (best[i].getStrategy() == strategy)
				devNumber++;
		}
	...
	public float getTop20AverageDownloads(Strategy strategy) {
		...
		for (int i = 0; i < bestAvg.length; i++) {
			if (bestAvg[i].getStrategy() == strategy)
				devNumber++;
		}
		return ((float) devNumber) / 20;
	}
	...
	public float getZeroDownloads(Strategy strategy) {
		int zero = 0;
		for (Developer dev : this.developers) {
			if (dev.getTotalDownloads() == 0 && dev.getStrategy() == strategy)
				zero++;
		}
